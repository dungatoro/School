...
while True:
	...
	if start != old_start or end != old_end:
		...
		try:
			...
			for x, y in path:
				canvas = tk.Canvas(buttons[y][x], bg=buttons[y][x].cget('bg'), width=button_dim, height=button_dim, highlightthickness=0)
				canvas.create_oval(8, 8, button_dim-4, button_dim-4, fill='white', outline='white')
